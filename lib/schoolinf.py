...
# учитель, а ученик вписывает один раз в поле «Журнал» своей копии.
...
def _из_секрета():
    """Настройки из «Секретов» Colab. Ни диалогов, ни разрешений."""
    try:
        from google.colab import userdata  # noqa: WPS433 (только в Colab)
    except ImportError:
        return None
    try:
        значение = userdata.get(СЕКРЕТ)
    except Exception:
        # секрета нет либо этому ноутбуку не разрешён доступ к нему
        return None
    return _разобрать(значение) if значение else None
...
def _сохранённые():
    """Найденные настройки. Секрет спрашиваем однократно: он показывает
    ученику окно с разрешением, и мигать им на каждый вызов незачем."""
...
def запомнить(name="", klass="", journal=""):
    ...
    print("Чтобы не вводить это на каждом уроке, заведи секрет.")
    print()
    print("1. Слева в Colab нажми значок ключа 🔑")
    print("2. «Добавить новый секрет»")
    print(f"3. Имя:      {СЕКРЕТ}")
    print(f"4. Значение: {строка}")
    print("5. Включи переключатель «Доступ в блокноте»")
    print()
    print("В следующих уроках хватит одного разрешения — поля заполнятся сами.")
    return строка
